...
describe("NVR Token", function () {
    beforeEach(async function () {
        ...
        const addr2 = accounts[1].getAddress();
        const addr3 = accounts[2].getAddress();
        const addr4 = accounts[3].getAddress();
        const addr5 = accounts[4].getAddress();

        const NVRToken = await ethers.getContractFactory("NVR");
        token = await NVRToken.connect(accounts[0]).deploy(addr2, addr3, addr4, addr5);
